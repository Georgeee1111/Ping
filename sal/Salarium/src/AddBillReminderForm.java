...
public class AddBillReminderForm extends JFrame {
  ...
  private CheckConnection checkConnection; // Declare the CheckConnection object
  ...
  public AddBillReminderForm() {
    ...
    checkConnection = new CheckConnection(); // Initialize the CheckConnection object
  ...
  public void addBillReminder() {
    ...
    try {
      ...
      try (Connection connection = checkConnection.getConnection()) { // Use the CheckConnection object
        // Check if user with id 1 exists
        if (!userExists(connection, id)) {
          JOptionPane.showMessageDialog(this, "User with ID 1 does not exist. Please create a user first.", "Error",
        ...
        ex.printStackTrace();
        JOptionPane.showMessageDialog(this, "Failed to add bill reminder: " + ex.getMessage(), "Error",
            JOptionPane.ERROR_MESSAGE);
      }
  ...
  private void viewBillReminders() {
    try (Connection connection = checkConnection.getConnection()) { // Use the CheckConnection object
      ...
      while (resultSet.next()) {
        ...
        billReminderTableModel
            .addRow(new Object[] { billId, billName, amount, dueDate, frequency, isPaid ? "Yes" : "No" });
      }
      ...
      ex.printStackTrace();
      JOptionPane.showMessageDialog(this, "Failed to retrieve bill reminders: " + ex.getMessage(), "Error",
          JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  private void paidButtonActionPerformed(ActionEvent evt) {
    ...
    if (billID == -1) {
      return; // No bill selected, show error message already handled in getSelectedBillId
              // method
    }
  ...
  private void markBillAsPaid(int billID) {
    try {
      connection = checkConnection.getConnection(); // Use the CheckConnection object
      ...
      if (update > 0) {
        ...
        JOptionPane.showMessageDialog(this, "Failed to mark bill reminder as paid.", "Error",
            JOptionPane.ERROR_MESSAGE);
      }
    } catch (SQLException ex) {
      ex.printStackTrace();
  ...
  public void updateExpenseTracker(BigDecimal amount) {
    ...
    try {
      ...
      if (update > 0 && insert > 0) {
        ...
        JOptionPane.showMessageDialog(this, "Failed to record expense or update budget tracker.", "Error",
            JOptionPane.ERROR_MESSAGE);
      }

    } catch (SQLException ex) {
      ex.printStackTrace();
      JOptionPane.showMessageDialog(this, "Error updating budget tracker or recording expense.", "Error",
          JOptionPane.ERROR_MESSAGE);
    } finally {
      try {
        if (connection != null) {
